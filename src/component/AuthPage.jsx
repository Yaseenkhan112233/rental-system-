...
const AuthPage = () => {
  ...
  const handleAppleSignIn = async () => {
    ...
    try {
      ...
      // Get user info from credentials - important for Apple sign-in
      const userInfo = result.user.providerData[0];
      const displayName = userInfo.displayName || "Apple User";
      ...
      if (err.code === "auth/popup-closed-by-user") {
        ...
        setError(
          "Apple sign-in is not enabled. Please make sure it's properly configured in Firebase."
        );
      } else if (err.code === "auth/unauthorized-domain") {
        setError(
          "This domain is not authorized for Firebase Authentication. The site administrator needs to add this domain to the authorized domains list in the Firebase console."
